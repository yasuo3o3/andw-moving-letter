...
CSVファイルから「お客様の声」を一括インポートできます。

**管理画面からの操作（推奨）:**
1. WordPress管理画面で「お客様の声」→「CSV一括追加」へ移動
...
**WP-CLIでの操作:**
```
wp andw import voices.csv                # 基本的なインポート
wp andw import voices.csv --dry-run      # プレビュー実行
wp andw import --sample > sample.csv     # サンプルCSV生成
```

**CSVフォーマット:**
```
title,nickname,plan_title,plan_url,body,tour_code
"投稿タイトル","ニックネーム","プラン名","URL","お便り本文","ツアーコード"
```

- 必須フィールド: `title`, `body`
- オプション: `nickname`, `plan_title`, `plan_url`, `tour_code`
- すべて公開状態で投稿されます
- 管理画面からサンプルCSVをダウンロード可能
...
**WP-CLIコマンド:**
* `wp andw stats` - 統計情報表示
* `wp andw purge` - データ完全削除
* `wp andw import <file>` - CSV一括インポート
